...
func handleAIOracleSimulationsRetrieve(ctx context.Context, cmd *cli.Command) error {
	...
	_, err = client.AI.Oracle.Simulations.Get(ctx, cmd.Value("simulation-id").(string), options...)
	if err != nil {
		return err
	}
...
func handleAIOracleSimulationsDelete(ctx context.Context, cmd *cli.Command) error {
	...
	return client.AI.Oracle.Simulations.Delete(ctx, cmd.Value("simulation-id").(string), options...)
}
